...
func TestApplyHeaderOverrides(t *testing.T) {
	...
	h["anthropic-beta"] = []string{"claude-code-20250219,oauth-2025-04-20"}
...
func TestNormalizeHeaderOverrideCredentialsForAccountStripsIneligibleTypes(t *testing.T) {
	creds := map[string]any{
		"access_token":               "token",
		credKeyHeaderOverrideEnabled: true,
	...
	require.NoError(t, NormalizeHeaderOverrideCredentialsForAccount(PlatformOpenAI, AccountTypeOAuth, creds))
	require.Equal(t, "token", creds["access_token"])
	require.NotContains(t, creds, credKeyHeaderOverrideEnabled)
	require.NotContains(t, creds, credKeyHeaderOverrides)
}
