...
func isHeaderOverrideEligible(platform, accountType string) bool {
	return accountType == AccountTypeAPIKey &&
		(platform == PlatformAnthropic || platform == PlatformOpenAI)
}
...
func (a *Account) IsHeaderOverrideEligible() bool {
	...
	return isHeaderOverrideEligible(a.Platform, a.Type)
}
...
func NormalizeHeaderOverrideCredentialsForAccount(platform, accountType string, credentials map[string]any) error {
	if credentials == nil {
		return nil
	}
	if !isHeaderOverrideEligible(platform, accountType) {
		delete(credentials, credKeyHeaderOverrideEnabled)
		delete(credentials, credKeyHeaderOverrides)
		return nil
	}
	return NormalizeHeaderOverrideCredentials(credentials)
}

func hasHeaderOverrideCredentialConfig(credentials map[string]any) bool {
	if credentials == nil {
		return false
	}
	_, hasEnabled := credentials[credKeyHeaderOverrideEnabled]
	_, hasOverrides := credentials[credKeyHeaderOverrides]
	return hasEnabled || hasOverrides
}
