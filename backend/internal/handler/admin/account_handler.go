...
type accountSchedulerScoreService interface {
...
// scoreOpenAIAccountSchedulerPool computes scheduler score snapshots for OpenAI accounts in a pool.
// loadMap is a shared load snapshot for all relevant accounts; nil falls back to a batch fetch.
...
func (h *AccountHandler) buildOpenAIAccountSchedulerScores(
	...
	schedulerSvc, hasSchedulerSvc := h.adminService.(accountSchedulerScoreService)
	if hasSchedulerSvc {
		for _, groupID := range groupIDList {
			gid := groupID
			pool, err := schedulerSvc.ListOpenAISchedulableAccountsForSchedulerScore(ctx, &gid)
			if err != nil {
...
func (h *AccountHandler) listAccountSchedulerScoreFilterPool(
	...
	schedulerSvc, ok := h.adminService.(accountSchedulerScoreService)
	if !ok {
		return nil
	}
	accounts, err := schedulerSvc.ListAccountsForSchedulerScoreFilter(ctx, service.PlatformOpenAI, accountType, status, search, groupID, privacyMode)
	if err != nil {
