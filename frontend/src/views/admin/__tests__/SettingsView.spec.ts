...
describe("admin SettingsView payment visible method controls", () => {
 ...
 const paymentLinks = wrapper.findAll("a").filter((node) => node.attributes("href")?.includes("docs/PAYMENT"));
 ...
 it("submits advanced scheduler sticky, subscription, TopK, and weight overrides", async () => {
 getSettings.mockResolvedValueOnce({
 ...baseSettingsResponse,
 ...
 expect(wrapper.text()).toContain("粘性加权");
 expect(wrapper.text()).toContain("订阅优先");
 expect(wrapper.text()).toContain("调度权值覆盖");
