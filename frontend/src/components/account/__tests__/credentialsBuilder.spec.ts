...
describe('isHeaderOverrideEligible', () => {
 it('requires both a supported platform and API key account type', () => {
 expect(isHeaderOverrideEligible('anthropic', 'apikey')).toBe(true)
 expect(isHeaderOverrideEligible('openai', 'apikey')).toBe(true)
 expect(isHeaderOverrideEligible('anthropic', 'oauth')).toBe(false)
 expect(isHeaderOverrideEligible('openai', 'setup-token')).toBe(false)
 expect(isHeaderOverrideEligible('gemini', 'apikey')).toBe(false)
 })
})
...
describe('buildHeaderOverridesObject / splitHeaderOverridesObject', () => {
 it('lowercases names, trims values and drops empty-name or empty-value placeholder rows', () => {
 expect(
 buildHeaderOverridesObject([
