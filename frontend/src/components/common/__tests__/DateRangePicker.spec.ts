...
describe('DateRangePicker', () => {
 ...
 it('shifts the dropdown back inside the viewport when it would overflow right', async () => {
 const now = new Date()
 const today = formatLocalDate(now)
 ...
 if (element.classList.contains('date-picker-dropdown')) {
 return {
 x: 260,
 y: 48,
 left: 260,
 ...
 expect(wrapper.find('.date-picker-dropdown').attributes('style')).toContain('left: -196px')
 })
})
